...
#include "all.chelpers.step42ns.parser.hpp"
#include <immintrin.h>

#define AVX_SIZE_ 4

void AllSteps::step42ns_parser_first_avx(StepsParams &params, uint64_t nrows, uint64_t nrowsBatch)
{
...
void AllSteps::step42ns_parser_first(StepsParams &params, uint64_t nrows, uint64_t nrowsBatch)
{
#pragma omp parallel for
     for (uint64_t i = 0; i < nrows; i += nrowsBatch)
     {
...
void AllSteps::step42ns_parser_first_avx_jump(StepsParams &params, uint64_t nrows, uint64_t nrowsBatch)
{
...
#ifdef __AVX512__
void AllSteps::step42ns_parser_first_avx512(StepsParams &params, uint64_t nrows, uint64_t nrowsBatch)
{
#pragma omp parallel for
     for (uint64_t i = 0; i < nrows; i += nrowsBatch)
     {
          ...
          for (int kk = 0; kk < NOPS_; ++kk)
          {
               ...
               case 89: 
               {
                    ...
                    Goldilocks3::sub31c_avx512(tmp3[(args42[i_args])], &params.pols[0], Goldilocks::fromU64(args42[i_args + 5]), offsets1);
                    i_args += 6;
                    break;
               }
               default:
